...
const anthropic = new Anthropic({ apiKey: process.env.LLM_PROVIDER_API_KEY });
...
async function generateLLMRecommendations(
  ...
  orgName: string,
  teamName: string
): Promise<{ strengths: string[]; developmentAreas: string[]; actionSteps: string[] }> {
...
For ${country} context, be specific about cultural nuances (e.g. Power Distance norms, collectivism, relationship-building). Frame recommendations positively and practically. Each item should be 1-3 sentences.`;
  ...
  try {
    ...
    logger.error('LLM recommendations failed', err);
    return {
...
export async function generateReport(assessmentId: string, includeLLM: boolean = false): Promise<void> {
  logger.info('Generating report', { assessmentId, includeLLM });

  // Get assessment + scores + benchmarks
  const result = await query(
    `SELECT a.name as team_name, a.target_country, a.target_industry, a.response_count,
            o.name as org_name,
  ...
  if (result.rows.length === 0) throw new Error('Assessment not found');
  ...
  await query(
    `INSERT INTO reports (assessment_id, organization_id, status)
     SELECT $1, organization_id, 'generating' FROM assessments WHERE id = $1
     ON CONFLICT (assessment_id) DO UPDATE SET status = 'generating', updated_at = NOW()`,
    [assessmentId]
  ).catch(async () => {
    // Handle case where unique constraint doesn't exist on assessment_id
    await query(
      `UPDATE reports SET status = 'generating', updated_at = NOW() WHERE assessment_id = $1`,
      [assessmentId]
    );
  });
  ...
  if (includeLLM) {
    const llmResult = await generateLLMRecommendations(
      scores, benchmarks as any, row.target_country,
      row.target_industry || 'General', row.org_name, row.team_name
    );
  ...
  logger.info('Report generated', { assessmentId });
}
