...
function getEmailContent(type: EmailOptions['type'], data: Record<string, any>) {
...
export async function sendEmail(options: EmailOptions): Promise<void> {
  ...
  try {
    ...
    // Log success
    await query(
      `INSERT INTO email_log (to_email, subject, type, assessment_id, organization_id, status, sent_at)
       VALUES ($1, $2, $3, $4, $5, 'sent', NOW())`,
      [to, content.subject, type, assessmentId || null, organizationId || null]
    );

    logger.info('Email sent', { to, type });
  } catch (err: any) {
    logger.error('Email send failed', { to, type, error: err.message });

    await query(
      `INSERT INTO email_log (to_email, subject, type, assessment_id, organization_id, status, error_msg)
       VALUES ($1, $2, $3, $4, $5, 'failed', $6)`,
      [to, `[${type}]`, type, assessmentId || null, organizationId || null, err.message]
...
export async function processReminderQueue() {
  ...
  const pending = await query(
    `SELECT el.to_email, el.assessment_id, a.name as assessment_name, o.name as org_name, a.survey_token
     FROM email_log el
