...
exports.assessmentRouter.get('/', async (req, res) => {
    try {
        const result = await (0, db_1.query)(`SELECT a.*, ds.overall_score, ds.colour_band,
       ...
       ORDER BY a.created_at DESC`, [req.user.orgId]);
...
// ── Create assessment ──────────────────────────────────────────
exports.assessmentRouter.post('/', (0, express_validator_1.body)('name').trim().notEmpty(), (0, express_validator_1.body)('targetCountry').trim().notEmpty(), async (req, res) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty())
        return res.status(400).json({ errors: errors.array() });
    const { name, description, targetCountry, targetIndustry, dimensionWeights } = req.body;
    try {
        const result = await (0, db_1.query)(`INSERT INTO assessments (organization_id, created_by, name, description, target_country, target_industry, dimension_weights)
         VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`, [req.user.orgId, req.user.id, name, description || null, targetCountry,
...
exports.assessmentRouter.get('/:id', async (req, res) => {
    try {
        const result = await (0, db_1.query)(`SELECT a.*, ds.*, r.status as report_status, r.pdf_url, r.executive_summary,
              r.strengths, r.development_areas, r.action_steps
       FROM assessments a
       LEFT JOIN dimension_scores ds ON ds.assessment_id = a.id
       LEFT JOIN reports r ON r.assessment_id = a.id
       WHERE a.id = $1 AND a.organization_id = $2`, [req.params.id, req.user.orgId]);
...
// ── Invite participants ────────────────────────────────────────
exports.assessmentRouter.post('/:id/invite', async (req, res) => {
    const { emails } = req.body;
    if (!Array.isArray(emails) || emails.length === 0) {
        return res.status(400).json({ error: 'emails array required' });
    }
    try {
        const assessmentResult = await (0, db_1.query)(`SELECT a.*, o.name as org_name
       FROM assessments a
       ...
       WHERE a.id = $1 AND a.organization_id = $2`, [req.params.id, req.user.orgId]);
        ...
        const results = await Promise.allSettled(emails.map((email) => (0, emailService_1.sendEmail)({
            to: email,
            type: 'invite',
        ...
        for (const email of emails) {
            await (0, db_1.query)(`INSERT INTO email_log (to_email, subject, type, assessment_id, organization_id, status)
         VALUES ($1, $2, 'invite', $3, $4, 'sent')`, [email, `Complete your cultural readiness survey for ${assessment.name}`, assessment.id, req.user.orgId]);
        }
...
exports.assessmentRouter.post('/:id/compute-scores', async (req, res) => {
    try {
        const assessmentResult = await (0, db_1.query)(`SELECT a.*, b.power_distance as b_pd, b.individualism_collectivism as b_ic,
       ...
       WHERE a.id = $1 AND a.organization_id = $2`, [req.params.id, req.user.orgId]);
        ...
        const responsesResult = await (0, db_1.query)(`SELECT answers FROM responses WHERE assessment_id = $1 AND is_complete = true`, [req.params.id]);
        ...
        const result = (0, scoringEngine_1.computeCRIScore)(responses, weights, benchmarks);
       ...
       ON CONFLICT (assessment_id) DO UPDATE SET
        ...
        await (0, db_1.query)(`UPDATE assessments SET status = 'completed', updated_at = NOW() WHERE id = $1`, [req.params.id]);
        res.json(result);
    }
    catch (err) {
