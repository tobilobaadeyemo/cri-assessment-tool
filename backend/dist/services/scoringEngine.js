 ...
 */
function scoreQuestion(questionId, rawValue) {
 ...
 */
function normalizeToHundred(rawAvg) {
 ...
 */
function scoreSingleResponse(answers) {
    ...
    for (const [dimension, scores] of Object.entries(dimensionRawScores)) {
        if (scores.length > 0) {
            ...
            result[dimension] = avg; // raw 1-5 average, not yet normalized
        }
    }
    return result;
}
 ...
 */
function computeCRIScore(allResponses, weights = {}, benchmarks) {
    ...
    for (const response of allResponses) {
        ...
        for (const [dim, val] of Object.entries(scored)) {
            if (val !== undefined) {
                dimensionAggregates[dim].push(val);
            }
        }
    }
 ...
 */
async function scoreAssessment(assessmentId, responses, weights, benchmarks) {
